...
class ElizabethViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun selectTrip(tripId: Long) = session.selectTrip(tripId)
    fun closeTripDetail() = session.closeTripDetail()
    fun deleteSavedTrip(tripId: Long) = session.deleteSavedTrip(tripId)
